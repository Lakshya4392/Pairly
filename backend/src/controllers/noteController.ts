...
/**
 * 💖 Trigger Full-Screen Lock Animation for Partner
 */
export const sendLockScreenEffect = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.userId!;
    ...
    if (!user?.isPremium) {
      res.status(403).json({
        success: false,
        error: 'Premium feature',
        message: 'Lock Screen Effects are available with Pairly Premium',
        upgradeRequired: true,
      } as ApiResponse);
      return;
    }
    ...
    // Send FCM Data Message for full-screen intent
    if (partner.fcmToken) {
      const FCMService = (await import('../services/FCMService')).default;
      await FCMService.sendLockScreenAnimation(
        partner.fcmToken,
        "I'm missing you so much!",
        user.displayName
      );
      console.log(`💖 [FCM] Lock screen effect triggered for ${partner.displayName}`);
    }
    ...
    console.error('Send lock screen effect error:', error);
    res.status(500).json({
      success: false,
